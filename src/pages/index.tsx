...
const MainPage: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const path = `companies/${companyId}/instrument`
  ...
  async function handlePasswordChange(value: string) {
    ...
    if(value.length === 4 ) {
      if(checkPassword(value)) {
        try {
          set(ref(database, `companies/${companyId}/instrument/REQ`), "GET /dataparam.cgi?logon=" + value)
          router.push("/setup")
        } catch (error) {
          console.error(error)
        }
  ...
  async function downloadFile(date: string, saveNumber: string) {
    ...
    try {
      const snapshot = await get(ref(database, `companies/${companyId}/instrument/dadoshist_cgi_btrqh_${saveNumber}`))
  ...
  function updateHistory(saveNumber: string) {
    try {
      set(ref(database, `companies/${companyId}/instrument/REQ`), "GET /dadoshist.cgi?btrqh=" + saveNumber)
    } catch (error) {
      console.error(error)
    }
 }
